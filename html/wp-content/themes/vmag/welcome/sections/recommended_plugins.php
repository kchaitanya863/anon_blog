<?php
	...
	if(!empty($pro_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Pro Plugins', 'vmag'); ?></h4>
		<p><?php echo esc_html__('Take Advantage of some of our Premium Plugins', 'vmag'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($pro_plugins as $plugin) {

			$status = $this->vmag_plugin_active($plugin);
	...
	if(!empty($free_plugins)) {
		?>
		<h4 class="recomplug-title"><?php echo esc_html__('Free Plugins', 'vmag'); ?></h4>
		<p><?php echo esc_html__('These Free Plugins might be handy for you.', 'vmag'); ?></p>
		<div class="recomended-plugin-wrap">
		<?php
		foreach($free_plugins as $plugin) {
			$info = $this->vmag_call_plugin_api($plugin['slug']);

			$icon_url = $this->vmag_check_for_icon($info->icons);
			$status = $this->vmag_plugin_active($plugin);
			$btn_url = $this->vmag_plugin_generate_url($status, $plugin);
